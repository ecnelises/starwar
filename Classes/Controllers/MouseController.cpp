...
bool MouseController::init()
{
    ...
    this->setTag(Config::Tag::mouseController);
...
{
    // set max range to limit force
    constexpr float maxDistance = 250;
    if (!_active || _selectedBall.isNil()) {
        return;
    }
    
    // if not clear drawer, frame will show ghosting
    _drawer->clear();
    auto destPoint = getCurrentCursor(event);
    auto pointDiff = destPoint - _balls->getPosition(_selectedBall);
    ...
    bottom.scale(10);
    _drawer->drawTriangle(destPoint, ballCenter + bottom, ballCenter - bottom,
                          cocos2d::Color4F(0.3, 0.3, 0.3, 0.4));
}
